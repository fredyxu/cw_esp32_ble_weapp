...
1. ##### 地狱模式容错率
	...
	在开启地狱模式后，允许有一定的键入时长误差，这里按照正负百分比进行判断，最低为0%（即不能有一毫秒的误差），最高为100%。
	例如：单位时长设定为100毫秒，容错率设定为0.1。则键入的时长在90毫秒和110毫秒内，均视为有效的键入点。如果设定容错率为0，则必须键入时长为准确的100毫秒才会被认为是有效的键入了点。（祝设定容错率为0的你玩的开心~）
